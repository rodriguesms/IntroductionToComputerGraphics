import * as THREE from 'https://cdn.skypack.dev/pin/three@v0.133.1-a8rkd0QTHl2tMZXZJAEw/mode=imports/optimized/three.js';
import { OrbitControls } from 'https://cdn.skypack.dev/pin/three@v0.133.1-a8rkd0QTHl2tMZXZJAEw/mode=imports/unoptimized/examples/jsm/controls/OrbitControls.js'


let gateNearestPoint = new Image();
gateNearestPoint.src = 'data:image/png;base64,';
// textura do portão do Doom

let gateBilinear = new Image();
gateBilinear.src = 'data:image/png;base64,';
// textura do portão do Doom


let textureGateNearestPoint = new THREE.Texture(gateNearestPoint);
let textureGateBilinear = new THREE.Texture(gateBilinear);


gateNearestPoint.onload = () => {
  textureGateNearestPoint.needsUpdate = true;
  textureGateNearestPoint.magFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de magnificação.
  textureGateNearestPoint.minFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de minificação.
  textureGateNearestPoint.anisotropy = 1; // fator máximo de anisotropia para o filtro anisotrópico.
  textureGateNearestPoint.wrapS = THREE.RepeatWrapping;
  textureGateNearestPoint.wrapT = THREE.RepeatWrapping;
};

gateBilinear.onload = function() {
    textureGateBilinear.needsUpdate = true;
    textureGateBilinear.magFilter = THREE.LinearFilter; // filtro a ser utilizado em caso de magnificação.
    textureGateBilinear.minFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de minificação.
    textureGateBilinear.anisotropy = 1; // fator máximo de anisotropia para o filtro anisotrópico.
    textureGateBilinear.wrapS = THREE.RepeatWrapping;
    textureGateBilinear.wrapT = THREE.RepeatWrapping;
};


let sceneGateNearestPoint = new THREE.Scene();

let sceneGateBilinear = new THREE.Scene();


let cameraGateNearestPoint = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.01, 100);
cameraGateNearestPoint.position.z = 1.5;

let cameraGateBilinear = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.01, 100);
cameraGateBilinear.position.z = 1.5;

sceneGateNearestPoint.add(cameraGateNearestPoint);
sceneGateBilinear.add(cameraGateBilinear);


let rendererGateNearestPoint = new THREE.WebGLRenderer({canvas: document.getElementById("canvas1"), alpha: true});
rendererGateNearestPoint.setPixelRatio( window.devicePixelRatio );
rendererGateNearestPoint.setSize( 720, 360 );
document.getElementById("object").appendChild(rendererGateNearestPoint.domElement);

let rendererGateBilinear = new THREE.WebGLRenderer({canvas: document.getElementById("canvas2"), alpha: true});
rendererGateBilinear.setPixelRatio( window.devicePixelRatio );
rendererGateBilinear.setSize( 720, 360 );
document.getElementById("object").appendChild(rendererGateBilinear.domElement);


let controlsGateBilinear = new OrbitControls(cameraGateBilinear, rendererGateBilinear.domElement);
controlsGateBilinear.enableDamping = true;
controlsGateBilinear.dampingFactor = 0.2;
controlsGateBilinear.rotateSpeed = 0.05;
controlsGateBilinear.screenSpacePanning = true;

let controlsGateNearestPoint = new OrbitControls(cameraGateNearestPoint, rendererGateNearestPoint.domElement);
controlsGateNearestPoint.enableDamping = true;
controlsGateNearestPoint.dampingFactor = 0.2;
controlsGateNearestPoint.rotateSpeed = 0.05;
controlsGateNearestPoint.screenSpacePanning = true;


let geometryGateBilinear = new THREE.BoxGeometry(1, 1, 1);
let geometryGateNearestPoint = new THREE.BoxGeometry(1, 1, 1);


//----------------------------------------------------------------------------
// Criação das fontes de luz pontuais.
//----------------------------------------------------------------------------
var point_light1GateBilinear = new THREE.PointLight(0xffffff);
point_light1GateBilinear.position.set(-10, 10, 20);
sceneGateBilinear.add(point_light1GateBilinear);

var point_light1GateNearestPoint = new THREE.PointLight(0xffffff);
point_light1GateNearestPoint.position.set(-10, 10, 20);
sceneGateNearestPoint.add(point_light1GateNearestPoint);

var point_light2GateBilinear = new THREE.PointLight(0xffffff);
point_light2GateBilinear.position.set(10, 10, 10);
sceneGateBilinear.add(point_light2GateBilinear);

var point_light2GateNearestPoint = new THREE.PointLight(0xffffff);
point_light2GateNearestPoint.position.set(10, 10, 10);
sceneGateNearestPoint.add(point_light2GateNearestPoint);

var point_light3 = new THREE.PointLight(0x666666);
point_light3.position.set(0, -10, -10);
sceneGateBilinear.add(point_light3);

var point_light3GateNearestPoint = new THREE.PointLight(0x666666);
point_light3GateNearestPoint.position.set(0, -10, -10);
sceneGateNearestPoint.add(point_light3GateNearestPoint);


//----------------------------------------------------------------------------
// Criação do material difuso. A textura define a reflectância difusa (k_d) 
// do material.
//----------------------------------------------------------------------------
let materialGateBilinear = new THREE.MeshLambertMaterial({
    map: textureGateBilinear
});

let materialGateNearestPoint = new THREE.MeshLambertMaterial({
  map: textureGateNearestPoint
});


var object_meshGateBilinear = new THREE.Mesh(geometryGateBilinear, materialGateBilinear);
sceneGateBilinear.add(object_meshGateBilinear);

var object_meshGateNearestPoint = new THREE.Mesh(geometryGateNearestPoint, materialGateNearestPoint);
sceneGateNearestPoint.add(object_meshGateNearestPoint);


function renderGateBilinear() {
  requestAnimationFrame(renderGateBilinear);
  rendererGateBilinear.render(sceneGateBilinear, cameraGateBilinear);
  controlsGateBilinear.update()  
}

function renderGate(){
  requestAnimationFrame(renderGate);
  rendererGateNearestPoint.render(sceneGateNearestPoint, cameraGateNearestPoint);
  controlsGateNearestPoint.update()  
}

renderGate();

renderGateBilinear();
