import * as THREE from 'https://cdn.skypack.dev/pin/three@v0.133.1-a8rkd0QTHl2tMZXZJAEw/mode=imports/optimized/three.js';
import { OrbitControls } from 'https://cdn.skypack.dev/pin/three@v0.133.1-a8rkd0QTHl2tMZXZJAEw/mode=imports/unoptimized/examples/jsm/controls/OrbitControls.js'


let gate = new Image();
gate.src = 'data:image/png;base64,';
// textura do portão do Doom

let chess = new Image();
chess.src = 'data:image/png;base64,'; 
// texture do padrão xadrez


let textureGate = new THREE.Texture(gate);
let textureChess = new THREE.Texture(chess);


gate.onload = () => {
  textureGate.needsUpdate = true;
  textureGate.magFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de magnificação.
  textureGate.minFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de minificação.
  textureGate.anisotropy = 1; // fator máximo de anisotropia para o filtro anisotrópico.
  textureGate.wrapS = THREE.RepeatWrapping;
  textureGate.wrapT = THREE.RepeatWrapping;
};

chess.onload = function() {
    textureChess.needsUpdate = true;
    textureChess.magFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de magnificação.
    textureChess.minFilter = THREE.NearestFilter; // filtro a ser utilizado em caso de minificação.
    textureChess.anisotropy = 1; // fator máximo de anisotropia para o filtro anisotrópico.
    textureChess.wrapS = THREE.RepeatWrapping;
    textureChess.wrapT = THREE.RepeatWrapping;
};


let sceneGate = new THREE.Scene();

let sceneChess = new THREE.Scene();


let cameraGate = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.01, 100);
cameraGate.position.z = 1.5;

let cameraChess = new THREE.PerspectiveCamera(75, window.innerWidth/window.innerHeight, 0.01, 100);
cameraChess.position.z = 1.5;

sceneGate.add(cameraGate);
sceneChess.add(cameraChess);


let rendererGate = new THREE.WebGLRenderer({canvas: document.getElementById("gate"), alpha: true});
rendererGate.setPixelRatio( window.devicePixelRatio );
rendererGate.setSize( 720, 360 );
document.getElementById("object").appendChild(rendererGate.domElement);

let rendererChess = new THREE.WebGLRenderer({canvas: document.getElementById("chess"), alpha: true});
rendererChess.setPixelRatio( window.devicePixelRatio );
rendererChess.setSize( 720, 360 );
document.getElementById("object").appendChild(rendererChess.domElement);


let controlsChess = new OrbitControls(cameraChess, rendererChess.domElement);
controlsChess.enableDamping = true;
controlsChess.dampingFactor = 0.2;
controlsChess.rotateSpeed = 0.05;
controlsChess.screenSpacePanning = true;

let controlsGate = new OrbitControls(cameraGate, rendererGate.domElement);
controlsGate.enableDamping = true;
controlsGate.dampingFactor = 0.2;
controlsGate.rotateSpeed = 0.05;
controlsGate.screenSpacePanning = true;


let geometryChess = new THREE.BoxGeometry(1, 1, 1);
let geometryGate = new THREE.BoxGeometry(1, 1, 1);


//----------------------------------------------------------------------------
// Criação das fontes de luz pontuais.
//----------------------------------------------------------------------------
var point_light1Chess = new THREE.PointLight(0xffffff);
point_light1Chess.position.set(-10, 10, 20);
sceneChess.add(point_light1Chess);

var point_light1Gate = new THREE.PointLight(0xffffff);
point_light1Gate.position.set(-10, 10, 20);
sceneGate.add(point_light1Gate);

var point_light2Chess = new THREE.PointLight(0xffffff);
point_light2Chess.position.set(10, 10, 10);
sceneChess.add(point_light2Chess);

var point_light2Gate = new THREE.PointLight(0xffffff);
point_light2Gate.position.set(10, 10, 10);
sceneGate.add(point_light2Gate);

var point_light3 = new THREE.PointLight(0x666666);
point_light3.position.set(0, -10, -10);
sceneChess.add(point_light3);

var point_light3Gate = new THREE.PointLight(0x666666);
point_light3Gate.position.set(0, -10, -10);
sceneGate.add(point_light3Gate);


//----------------------------------------------------------------------------
// Criação do material difuso. A textura define a reflectância difusa (k_d) 
// do material.
//----------------------------------------------------------------------------
let materialChess = new THREE.MeshLambertMaterial({
    map: textureChess
});

let materialGate = new THREE.MeshLambertMaterial({
  map: textureGate
});


var object_meshChess = new THREE.Mesh(geometryChess, materialChess);
sceneChess.add(object_meshChess);

var object_meshGate = new THREE.Mesh(geometryGate, materialGate);
sceneGate.add(object_meshGate);


function renderChess() {
  requestAnimationFrame(renderChess);
  rendererChess.render(sceneChess, cameraChess);
  controlsChess.update()  
}

function renderGate(){
  requestAnimationFrame(renderGate);
  rendererGate.render(sceneGate, cameraGate);
  controlsGate.update()  
}

renderGate();

renderChess();
